...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/echoscu' });
...
describe('echoscu', () => {
    describe('validation', () => {
        ...
        it('rejects invalid port', async () => {
            const result = await echoscu({ host: 'localhost', port: 0 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduReceive below 4096', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, maxPduReceive: 1024 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduReceive above 131072', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, maxPduReceive: 200000 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduSend below 4096', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, maxPduSend: 100 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduSend above 131072', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, maxPduSend: 999999 });
            expect(result.ok).toBe(false);
        });

        it('rejects associationTimeout less than 1', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, associationTimeout: 0 });
            expect(result.ok).toBe(false);
        });

        it('rejects acseTimeout less than 1', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, acseTimeout: 0 });
            expect(result.ok).toBe(false);
        });

        it('rejects dimseTimeout less than 1', async () => {
            const result = await echoscu({ host: 'localhost', port: 104, dimseTimeout: 0 });
            expect(result.ok).toBe(false);
        });
    });
    ...
    describe('argument building', () => {
        ...
        it('passes --max-pdu with value for maxPduReceive', async () => {
            await echoscu({ host: 'localhost', port: 104, maxPduReceive: 16384 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes --max-send-pdu with value for maxPduSend', async () => {
            await echoscu({ host: 'localhost', port: 104, maxPduSend: 65536 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -to with value for associationTimeout', async () => {
            await echoscu({ host: 'localhost', port: 104, associationTimeout: 30 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -ta with value for acseTimeout', async () => {
            await echoscu({ host: 'localhost', port: 104, acseTimeout: 15 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -td with value for dimseTimeout', async () => {
            await echoscu({ host: 'localhost', port: 104, dimseTimeout: 60 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -nh for noHostnameLookup', async () => {
            await echoscu({ host: 'localhost', port: 104, noHostnameLookup: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-nh');
        });

        it('includes AE title flags', async () => {
            await echoscu({ host: 'localhost', port: 104, callingAETitle: 'MYSCU', calledAETitle: 'PACS' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
