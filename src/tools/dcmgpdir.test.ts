...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/dcmgpdir' });
...
describe('dcmgpdir', () => {
    ...
    describe('result handling', () => {
        it('returns outputPath on success', async () => {
            const result = await dcmgpdir({ inputFiles: ['/file.dcm'], outputFile: '/out/DICOMDIR' });
        ...
        it('defaults outputPath to DICOMDIR when outputFile not specified', async () => {
            const result = await dcmgpdir({ inputFiles: ['/file.dcm'] });
            expect(result.ok).toBe(true);
            if (result.ok) {
