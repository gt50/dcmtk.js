...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/xml2dsr' });
...
describe('xml2dsr', () => {
    describe('argument building', () => {
        it('passes -v for verbose', async () => {
            await xml2dsr('/input.xml', '/output.dcm', { verbosity: 'verbose' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-v');
        });

        it('passes -d for debug', async () => {
            await xml2dsr('/input.xml', '/output.dcm', { verbosity: 'debug' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-d');
        });

        it('omits verbosity when not specified', async () => {
            await xml2dsr('/input.xml', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes +Ug for generateNewUIDs', async () => {
            await xml2dsr('/input.xml', '/output.dcm', { generateNewUIDs: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('+Ug');
        });

        it('passes +Vd for validateDocument', async () => {
            await xml2dsr('/input.xml', '/output.dcm', { validateDocument: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
    ...
    describe('result handling', () => {
        it('returns success with outputPath', async () => {
            const result = await xml2dsr('/input.xml', '/output.dcm');
        ...
        it('returns error on non-zero exit', async () => {
            mockedExecCommand.mockResolvedValue({
                ok: true,
                value: { stdout: '', stderr: 'parse error', exitCode: 1 },
            });
            const result = await xml2dsr('/input.xml', '/output.dcm');
