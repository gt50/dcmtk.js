...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/drtdump' });
    mockedExecCommand.mockResolvedValue({
        ok: true,
        value: { stdout: 'RT dump output', stderr: '', exitCode: 0 },
    });
});
...
describe('drtdump', () => {
    describe('argument building', () => {
        ...
        it('passes +Pf for printFilename', async () => {
            await drtdump('/rtplan.dcm', { printFilename: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('includes input path at the end', async () => {
            await drtdump('/rtplan.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args[args.length - 1]).toBe('/rtplan.dcm');
        });
    });

    describe('result handling', () => {
        it('returns text on success', async () => {
            const result = await drtdump('/rtplan.dcm');
            expect(result.ok).toBe(true);
            if (result.ok) {
                expect(result.value.text).toBe('RT dump output');
            }
        });
