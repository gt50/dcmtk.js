...
describe('dcm2pnm', () => {
    describe('validation', () => {
        it('rejects windowCenter without windowWidth', async () => {
            const result = await dcm2pnm('/input.dcm', '/output.png', { windowCenter: 128 });
            expect(result.ok).toBe(false);
        });

        it('rejects windowWidth without windowCenter', async () => {
            const result = await dcm2pnm('/input.dcm', '/output.png', { windowWidth: 256 });
            expect(result.ok).toBe(false);
        });

        it('accepts windowCenter and windowWidth together', async () => {
            const result = await dcm2pnm('/input.dcm', '/output.png', { windowCenter: 128, windowWidth: 256 });
            expect(result.ok).toBe(true);
        });

        it('accepts all output format values', async () => {
            for (const format of Object.values(Dcm2pnmOutputFormat)) {
                const result = await dcm2pnm('/input.dcm', '/output.img', { outputFormat: format });
                expect(result.ok).toBe(true);
            }
        });

        it('rejects invalid output format', async () => {
            const result = await dcm2pnm('/input.dcm', '/output.img', { outputFormat: 'gif' as never });
            expect(result.ok).toBe(false);
        });
    });
    ...
    describe('argument building', () => {
        ...
        it('passes +on2 for PNG_16BIT', async () => {
            await dcm2pnm('/input.dcm', '/output.png', { outputFormat: Dcm2pnmOutputFormat.PNG_16BIT });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('+on2');
        });

        it('passes +on for PNG', async () => {
            await dcm2pnm('/input.dcm', '/output.png', { outputFormat: Dcm2pnmOutputFormat.PNG });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('+on');
        });

        it('passes +Wl with center and width values', async () => {
            await dcm2pnm('/input.dcm', '/output.png', { windowCenter: 128, windowWidth: 256 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes +F with frame number', async () => {
            await dcm2pnm('/input.dcm', '/output.png', { frame: 5 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
    ...
    describe('binary fallback', () => {
        ...
        it('falls back to dcm2pnm when dcm2img not found', async () => {
            mockedResolveBinary
                .mockReturnValueOnce({ ok: false, error: new Error('dcm2img not found') })
                .mockReturnValueOnce({ ok: true, value: '/usr/local/bin/dcm2pnm' });
            await dcm2pnm('/input.dcm', '/output.png');
            expect(mockedResolveBinary).toHaveBeenCalledWith('dcm2img');
            expect(mockedResolveBinary).toHaveBeenCalledWith('dcm2pnm');
            expect(mockedExecCommand.mock.calls[0]?.[0]).toBe('/usr/local/bin/dcm2pnm');
        });
        ...
        it('returns error when neither binary is found', async () => {
            ...
            const result = await dcm2pnm('/input.dcm', '/output.png');
            expect(result.ok).toBe(false);
        });
    });
    ...
    describe('Dcm2pnmOutputFormat constants', () => {
        it('has all 5 values', () => {
            expect(Object.keys(Dcm2pnmOutputFormat)).toHaveLength(5);
        });

        it('includes PNG_16BIT', () => {
            expect(Dcm2pnmOutputFormat.PNG_16BIT).toBe('png16');
        });

        it('values are unique strings', () => {
            const values = Object.values(Dcm2pnmOutputFormat);
