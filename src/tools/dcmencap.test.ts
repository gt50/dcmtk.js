...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/dcmencap' });
...
describe('dcmencap', () => {
    describe('argument building', () => {
        it('passes -v for verbose', async () => {
            await dcmencap('/input.pdf', '/output.dcm', { verbosity: 'verbose' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-v');
        });

        it('passes -d for debug', async () => {
            await dcmencap('/input.pdf', '/output.dcm', { verbosity: 'debug' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-d');
        });

        it('omits verbosity when not specified', async () => {
            await dcmencap('/input.pdf', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).not.toContain('-v');
            expect(args).not.toContain('-d');
        });

        it('passes --title with document title', async () => {
            await dcmencap('/input.pdf', '/output.dcm', { documentTitle: 'My Document' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            const idx = args.indexOf('--title');
            expect(idx).toBeGreaterThanOrEqual(0);
            expect(args[idx + 1]).toBe('My Document');
        });

        it('includes input and output paths at the end', async () => {
            await dcmencap('/input.pdf', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args[args.length - 2]).toBe('/input.pdf');
    ...
    describe('result handling', () => {
        it('returns outputPath on success', async () => {
            const result = await dcmencap('/input.pdf', '/output.dcm');
        ...
        it('returns error on non-zero exit code', async () => {
            mockedExecCommand.mockResolvedValue({
                ok: true,
                value: { stdout: '', stderr: 'encapsulation failed', exitCode: 1 },
            });
            const result = await dcmencap('/input.pdf', '/output.dcm');
