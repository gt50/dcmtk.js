...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/dcmdrle' });
...
describe('dcmdrle', () => {
    describe('argument building', () => {
        it('passes -v for verbose', async () => {
            await dcmdrle('/input.dcm', '/output.dcm', { verbosity: 'verbose' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-v');
        });

        it('passes -d for debug', async () => {
            await dcmdrle('/input.dcm', '/output.dcm', { verbosity: 'debug' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-d');
        });

        it('omits verbosity when not specified', async () => {
            await dcmdrle('/input.dcm', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes +ua for uidAlways', async () => {
            await dcmdrle('/input.dcm', '/output.dcm', { uidAlways: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
    ...
    describe('result handling', () => {
        it('returns outputPath on success', async () => {
            const result = await dcmdrle('/input.dcm', '/output.dcm');
        ...
        it('returns error on non-zero exit code', async () => {
            mockedExecCommand.mockResolvedValue({
                ok: true,
                value: { stdout: '', stderr: 'decoding failed', exitCode: 1 },
            });
            const result = await dcmdrle('/input.dcm', '/output.dcm');
