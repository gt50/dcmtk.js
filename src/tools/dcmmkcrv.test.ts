...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/dcmmkcrv' });
...
describe('dcmmkcrv', () => {
    describe('argument building', () => {
        it('passes -v for verbose', async () => {
            await dcmmkcrv('/curves.txt', '/output.dcm', { verbosity: 'verbose' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-v');
        });

        it('passes -d for debug', async () => {
            await dcmmkcrv('/curves.txt', '/output.dcm', { verbosity: 'debug' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-d');
        });

        it('omits verbosity when not specified', async () => {
            await dcmmkcrv('/curves.txt', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('includes input and output paths at the end', async () => {
            await dcmmkcrv('/curves.txt', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args[args.length - 2]).toBe('/curves.txt');
    ...
    describe('result handling', () => {
        it('returns success with outputPath', async () => {
            const result = await dcmmkcrv('/curves.txt', '/output.dcm');
