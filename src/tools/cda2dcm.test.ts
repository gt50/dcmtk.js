...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/cda2dcm' });
...
describe('cda2dcm', () => {
    describe('argument building', () => {
        ...
        it('includes input and output paths at the end', async () => {
            await cda2dcm('/input.cda', '/output.dcm');
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args[args.length - 2]).toBe('/input.cda');
    ...
    describe('result handling', () => {
        it('returns outputPath on success', async () => {
            const result = await cda2dcm('/input.cda', '/output.dcm');
