...
beforeEach(() => {
    vi.clearAllMocks();
    mockedResolveBinary.mockReturnValue({ ok: true, value: '/usr/local/bin/findscu' });
...
describe('findscu', () => {
    describe('validation', () => {
        ...
        it('rejects extract without outputDirectory', async () => {
            const result = await findscu({ host: 'localhost', port: 104, extract: true });
            expect(result.ok).toBe(false);
        });

        it('accepts extract with outputDirectory', async () => {
            const result = await findscu({ host: 'localhost', port: 104, extract: true, outputDirectory: '/tmp' });
            expect(result.ok).toBe(true);
        });

        it('rejects maxPduReceive below 4096', async () => {
            const result = await findscu({ host: 'localhost', port: 104, maxPduReceive: 1024 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduReceive above 131072', async () => {
            const result = await findscu({ host: 'localhost', port: 104, maxPduReceive: 200000 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduSend below 4096', async () => {
            const result = await findscu({ host: 'localhost', port: 104, maxPduSend: 100 });
            expect(result.ok).toBe(false);
        });

        it('rejects maxPduSend above 131072', async () => {
            const result = await findscu({ host: 'localhost', port: 104, maxPduSend: 999999 });
            expect(result.ok).toBe(false);
        });

        it('rejects associationTimeout less than 1', async () => {
            const result = await findscu({ host: 'localhost', port: 104, associationTimeout: 0 });
            expect(result.ok).toBe(false);
        });

        it('rejects acseTimeout less than 1', async () => {
            const result = await findscu({ host: 'localhost', port: 104, acseTimeout: 0 });
            expect(result.ok).toBe(false);
        });

        it('rejects dimseTimeout less than 1', async () => {
            const result = await findscu({ host: 'localhost', port: 104, dimseTimeout: 0 });
            expect(result.ok).toBe(false);
        });
    });
    ...
    describe('argument building', () => {
        ...
        it('passes --max-pdu with value for maxPduReceive', async () => {
            await findscu({ host: 'localhost', port: 104, maxPduReceive: 16384 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes --max-send-pdu with value for maxPduSend', async () => {
            await findscu({ host: 'localhost', port: 104, maxPduSend: 65536 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -to with value for associationTimeout', async () => {
            await findscu({ host: 'localhost', port: 104, associationTimeout: 30 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -ta with value for acseTimeout', async () => {
            await findscu({ host: 'localhost', port: 104, acseTimeout: 15 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -td with value for dimseTimeout', async () => {
            await findscu({ host: 'localhost', port: 104, dimseTimeout: 60 });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
        ...
        it('passes -nh for noHostnameLookup', async () => {
            await findscu({ host: 'localhost', port: 104, noHostnameLookup: true });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-nh');
        });

        it('passes query model flag', async () => {
            await findscu({ host: 'localhost', port: 104, queryModel: 'study' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            expect(args).toContain('-S');
        });

        it('passes -k for each key', async () => {
            await findscu({ host: 'localhost', port: 104, keys: ['0008,0050=', '0010,0020=PAT001'] });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
            const firstIdx = args.indexOf('-k');
            expect(firstIdx).toBeGreaterThanOrEqual(0);
            expect(args[firstIdx + 1]).toBe('0008,0050=');
            const secondIdx = args.indexOf('-k', firstIdx + 1);
            expect(secondIdx).toBeGreaterThanOrEqual(0);
            expect(args[secondIdx + 1]).toBe('0010,0020=PAT001');
        });

        it('includes AE title flags', async () => {
            await findscu({ host: 'localhost', port: 104, callingAETitle: 'MYSCU', calledAETitle: 'PACS' });
            const args = mockedExecCommand.mock.calls[0]?.[1] as string[];
