...
| `parseInstances`      | `boolean`           | `true`                          | Parse files into DicomInstance. When false, no INSTANCE_RECEIVED/INSTANCE_ERROR |
...
| `ASSOCIATION_FINALIZED` | `{ associationId, associationDir, callingAE, calledAE, source, files, instancesReceived, instanceErrors, endReason }`                              | **All work done** — file moves + parsing complete; includes success/error counts and `endReason` (`'release'` \| `'abort'`) |
...
receiver.onFileStored(data => {
    console.log(`Stored: ${data.filePath} (${data.fileSize} bytes)`);
});
